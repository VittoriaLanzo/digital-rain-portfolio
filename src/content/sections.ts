...
Sestara is the system she never had: an AI-powered edtech platform that turns any exam goal into a structured, trackable, and emotionally supported study journey — with the precision of a private tutor and the accessibility of a free tool. It generates a complete, specific curriculum for any exam, tracks every topic in real time, quizzes students with explanations, and deploys an AI companion that knows exactly where they stand.

## For Product Designers

Every surface of Sestara was designed to feel like a private tutor, not a productivity app. The distinction matters: productivity apps optimise for completion. Tutors optimise for understanding. That single design principle propagates through every screen — the dashboard as a command center, progress as a live roadmap, quizzes that explain not just correct and wrong but why, and an AI companion that responds differently depending on whether a student is ahead, on track, or three weeks behind.
...
Fabric.js powers an in-app drawing canvas. Students preparing for visual subjects — anatomy diagrams, circuit schematics, geometric proofs — can sketch and annotate directly inside Sestara without switching tools. The architecture is modular by design: each feature is a self-contained unit sharing one Lovable Cloud backend, which made parallel development between two contributors across two countries clean, fast, and conflict-free. Zod validates every JSON schema exchange between the AI layer and the application, so malformed language model outputs never crash a user session mid-use.

GDPR compliance was built in from day one, not retrofitted. Lovable Cloud processors are bound by Article 28 Data Processing Agreements. All data is encrypted in transit. No student data is resold. The compliance architecture was documented before the first feature was shipped, because in a European edtech product, trust is infrastructure.
...
The emotional register of StudyBuddy was engineered with the same precision as its knowledge scope. The assistant checks in. It notices when a student is three weeks behind schedule and addresses it directly rather than just answering the surface question. The design intent is specific: the difference between a student who quits and one who pushes through is rarely intelligence. It is having someone in their corner at 2 AM the night before the exam. The system prompt encodes that person — their tone, their awareness, their conditions for offering encouragement versus challenge.

The Custom Quiz Studio extends this architecture outward. Sestara generates a ready-to-use prompt with a structured JSON schema that any language model can parse — ChatGPT, Claude, Gemini, Llama, Perplexity. The schema specifies question format, difficulty tier, topic scope, explanation depth, and answer structure. The student pastes the model's output back; Sestara parses it via Zod validation and feeds it into the progress system. The quiz library grows with every session. No external provider is required and no provider is privileged — an AI-agnostic stance taken deliberately to ensure every student, regardless of what tools their school or device supports, has the same Sestara experience.`,
